...
var handles1 = ["SELECT CATEGORY","Crime against women","Murder","Theft","Kidnapping"];
console.log("inside category0");
$(function() {
  var options = '';
  for (var i = 0; i < handles1.length; i++) {
      options += '<option value="' + handles1[i] + '">' + handles1[i] + '</option>';
  }
  $('#listBox2').html(options);

});
function selct_subcategory($val)
{
 ...
 if($val=='Murder') {
   var murder = ["Murder","Murder by life-convict","Culpable homicide not amounting to murder","Causing death by negligence","Attempt to murder"];
   $(function() {
  var options = '';
  for (var i = 0; i < murder.length; i++) {
      options += '<option value="' + murder[i] + '">' + murder[i] + '</option>';
  }
  $('#secondlist2').html(options);
  });
  }
  ...
  if ($val=='Theft') {
    var ap1 = ["Theft","Theft in dwelling house, etc.","Theft after preparation made for causing death, hurt or restraint in order to the committing of the theft.","Robbery","Attempt to robbery","Voluntarily causing hurt in committing robbery","Dacoity","Dacoity with murder"];
   $(function() {
  var options = '';
  for (var i = 0; i < ap.length; i++) {
      options += '<option value="' + ap1[i] + '">' + ap1[i] + '</option>';
  }
  $('#secondlist2').html(options);
  });
  }
  ...
  if ($val=='Kidnapping') {
    var kidnap = ["kidnapping","Kidnapping for ransom etc","Kidnapping or abducting in order to murder"];
   $(function() {
  var options = '';
  for (var i = 0; i < kidnap.length; i++) {
      options += '<option value="' + kidnap[i] + '">' + kidnap[i] + '</option>';
  }
  $('#secondlist2').html(options);
  });
  }
